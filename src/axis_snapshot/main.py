...
def parse_args(args):
    ...
    parser.add_argument(
        "-n", "--name", dest="name", help="location name", metavar="NAME"
    )
    parser.add_argument(
        "-f",
        "--force",
    ...
    parser.add_argument(
        "-l", "--latitude", dest="latitude", help="latitude", metavar="FLOAT"
    )  # 35.800783
    parser.add_argument(
        "-g", "--longitude", dest="longitude", help="longitude", metavar="FLOAT"
    )  # -78.645814
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(dest="url", help="camera URL", metavar="URL")
...
def main(args):
    ...
    if args.force == True or (now > today.sunrise_time and now < today.sunset_time):
        ...
        r = requests.get(
            args.url, auth=HTTPDigestAuth(username, password), verify=False, stream=True
        )
        if r.status_code == requests.codes.ok:
